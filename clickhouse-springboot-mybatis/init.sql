...
-- 插入用户数据
INSERT INTO demo.user (id, name) VALUES
                                     (1, '张三'),
                                     (2, '李四'),
                                     (3, '王五'),
                                     (4, '赵六'),
...
-- 插入用户登录日志（多天数据）
INSERT INTO demo.userlog (user_id, login_time) VALUES
-- 用户1的登录记录
...
SELECT
    ...
    ul.login_time
FROM demo.user u
         JOIN demo.userlog ul ON u.id = ul.user_id
...
FROM demo.user u
         LEFT JOIN demo.userlog ul ON u.id = ul.user_id
